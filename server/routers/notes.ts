...
import { notes, diaries, streaks, loginLogs, schedules, classificationLogs, customCategories } from "../../drizzle/schema";
...
- 未归类输入 (subCategory: "uncategorized")：确定是输入类，但无法匹配以上任何子类时使用
...
- 未归类输出 (subCategory: "uncategorized")：确定是输出类，但无法匹配以上任何子类时使用
...
async function classifyNote(noteId: number, rawText: string, userId: number) {
  ...
  try {
    ...
    let customCatPrompt = '';
    try {
      const userCustomCats = await db
        .select()
        .from(customCategories)
        .where(eq(customCategories.userId, userId));
      if (userCustomCats.length > 0) {
        const inputCats = userCustomCats.filter(c => c.parentCategory === 'input');
        const outputCats = userCustomCats.filter(c => c.parentCategory === 'output');
        if (inputCats.length > 0 || outputCats.length > 0) {
          customCatPrompt = '\n\n【用户自定义分类（优先匹配）】\n';
          if (inputCats.length > 0) {
            customCatPrompt += '输入类自定义子分类：\n';
            inputCats.forEach(c => {
              customCatPrompt += `- ${c.label} (subCategory: "${c.subCategory}")\n`;
            });
          }
          if (outputCats.length > 0) {
            customCatPrompt += '输出类自定义子分类：\n';
            outputCats.forEach(c => {
              customCatPrompt += `- ${c.label} (subCategory: "${c.subCategory}")\n`;
            });
          }
          customCatPrompt += '如果内容与以上自定义分类高度匹配，请优先使用自定义分类的 subCategory。';
        }
      }
    } catch (e) {
      // 查询失败不阻断分类
    ...
    const response = await invokeLLM({
      messages: [
        { role: "system", content: CLASSIFY_SYSTEM_PROMPT + customCatPrompt + `\n\n【重要】今天的日期是 ${todayStr}，请以此为基准推算所有相对日期（如"今天"、"明天"、"下周五"等）。` },
